...
class TestLeaderboardValidation:
    ...
    def test_rejects_non_object_body(self, client):
        res = client.post("/leaderboard", json=["Player"])
        assert res.status_code == 400
...
class TestSessionCreation:
    ...
    def test_non_object_body_rejected(self, client):
        res = client.post("/session", json=["medium", 5])
        assert res.status_code == 400

    def test_boolean_max_rounds_rejected(self, client):
        res = client.post("/session", json={"difficulty": "medium", "max_rounds": True})
        assert res.status_code == 400

    @pytest.mark.parametrize("field", ["leaderboard_mode", "outside_only"])
    def test_non_boolean_flags_rejected(self, client, field):
        res = client.post("/session", json={field: "true"})
        assert res.status_code == 400

    def test_non_string_seed_rejected(self, client):
        res = client.post("/session", json={"seed": 123})
        assert res.status_code == 400
...
class TestRandomImage:
    ...
    def test_new_round_stores_internal_id_and_returns_direct_cdn_url(self, client, app):
        ...
        assert set(data) == {"image_url", "round_number"}
...
class TestGuessValidation:
    def test_rejects_non_object_body(self, client):
        res = client.post("/guess", json=["test-session-123", 1])
        assert res.status_code == 400

    def test_rejects_boolean_round_number(self, client):
    ...
    def test_rejects_non_numeric_coordinates(self, client):
        res = client.post("/guess", json={
            "session_id": "test-session-123",
            "round_number": 1,
            "guess_latitude": "10",
